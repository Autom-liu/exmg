<template>
  <view>
    ...
    <view>
		<image class="uni-header-image" src="https://pic.5tu.cn/uploads/allimg/1911/pic_5tu_big_201911110937385525.jpg"></image>
    </view>
    <div class="category-bar" @touchstart="startDrag" @touchmove="drag" @touchend="dragEnd">
      <div class="category-override" :style="categoryTabsAttr.overrideStyle">
        <div class="slider" :style="categoryTabsAttr.sliderStyle" ref="slider">
          <!-- 忽视报错,在小程序中需要这样的写法-->
          <div class="category" :style="categoryTabsAttr.itemStyle" v-for="category in categoryField.categories" :key="category.code" @click="toRandomExercise(category)">
            <img class="img" src="/static/images/database.png" alt="图标">
      ...
      <div class="list-inner">
        ...
        <div v-for="item in examList" :key="item.id" class="list-table" @click="toQuestion(item)">
          ...
          <div class="arrow-container">
            <image src="/static/images/arrowright.png" class="arrow-icon" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    toQuestion (row) {
		uni.navigateTo({
			url:`/pages/prepareEnter/main?query=${JSON.stringify(row)}`
		})
    },
    toRandomExercise (row) {
		uni.navigateTo({
			url:`/pages/randomExercise/main?query=${JSON.stringify(row)}`
		})
    },
    initPage () {
      http.wxlogin()
